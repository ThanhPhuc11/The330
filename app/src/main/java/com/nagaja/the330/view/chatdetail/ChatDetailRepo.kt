...
class ChatDetailRepo(private val apiService: ApiService) {
    ...
    suspend fun getChatDetail(token: String, roomId: Int, chatMesId: Int?, size: Int) = flow {
        emit(apiService.getChatDetail(token, chatRoomId = roomId, chatMesId = chatMesId, size = size))
    }
    ...
    suspend fun endChat(token: String, roomId: Int) = flow {
        emit(apiService.endChat(token, roomId))
    }
}